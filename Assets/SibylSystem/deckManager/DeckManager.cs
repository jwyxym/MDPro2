...
public class DeckManager : ServantWithCardDescription
{
    ...
    private number_loader extra_number;

    private GameObject gameObjectDesk;
    private GameObject goLast;

    private number_loader m_number;

    private number_loader main_number;

    private number_loader s_number;

    private number_loader side_number;

    private number_loader t_number;
    ...
    public override void show()
    {
        ...
        m_number = create_s(Program.I().mod_ocgcore_number, new Vector3(-16.5f, 0, 6.6f), new Vector3(90, 0, 0), true)
            .GetComponent<number_loader>();
        s_number = create_s(Program.I().mod_ocgcore_number, new Vector3(-16.5f, 0, 4.6f), new Vector3(90, 0, 0), true)
            .GetComponent<number_loader>();
        t_number = create_s(Program.I().mod_ocgcore_number, new Vector3(-16.5f, 0, 2.6f), new Vector3(90, 0, 0), true)
            .GetComponent<number_loader>();
        extra_number =
        ...
        side_number =
            create_s(Program.I().mod_ocgcore_number, new Vector3(-16.5f, 0, -11f), new Vector3(90, 0, 0), true)
                .GetComponent<number_loader>();
        main_number.gameObject.layer = 16;
        m_number.gameObject.layer = 16;
        s_number.gameObject.layer = 16;
        t_number.gameObject.layer = 16;
        extra_number.gameObject.layer = 16;
        side_number.gameObject.layer = 16;
        switch (condition)
        {
    ...
    public override void preFrameFunction()
    {
        ...
        camrem();
        cameraDistance = 27 - 3.1415926f / 180f * (cameraAngle - 60f) * 13f;
        Program.cameraPosition = new Vector3(0, cameraDistance * Mathf.Sin(3.1415926f / 180f * cameraAngle) * 3, -cameraDistance * Mathf.Cos(3.1415926f / 180f * cameraAngle) * 3);
        if (Program.TimePassed() - lastRefreshTime > 80)
        {
            lastRefreshTime = Program.TimePassed();
            FromObjectDeckToCodedDeck();
            main_number.set_number(deck.Main.Count, 3);
            side_number.set_number(deck.Side.Count, 5);
            extra_number.set_number(deck.Extra.Count, 4);
            int m = 0, s = 0, t = 0;
            ...
            m_number.set_number(m, 1);
            s_number.set_number(s, 2);
            t_number.set_number(t, 0);
        }
    ...
    {
        Deck.sortMonoCard((List<MonoCardInDeckManager>) deck.IMain);
        Deck.sortMonoCard((List<MonoCardInDeckManager>) deck.IExtra);
        Deck.sortMonoCard((List<MonoCardInDeckManager>) deck.ISide);
        deckDirty = true;
    }
    ...
    {
        if (deck.Main != null && deck.Main.Count >0 &&deck.Main[0] != 0)
    ...
    public override void initialize()
    {
        ...
        for (var i = 0; i < 32; i++)
        {
            ...
            UIHelper.trySetLabelText(UIToggle_effects[i].gameObject, GameStringManager.get_unsafe(1100 + i));
    ...
    private GameObject itemOnListProducer(string[] args)
    {
        ...
        UIHelper.trySetLabelText(returnValue, args[2]);
