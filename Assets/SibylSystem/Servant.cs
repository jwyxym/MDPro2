...
public class Servant
{
    ...
    public void hideBarOnly()
    {
        ...
        {
            toolBar.transform.DOMove(
                Utils.UIToWorldPoint(new Vector3(Utils.UIWidth() - RightToScreen, -200, 0)),
                0.6f);
            var items = toolBar.GetComponentsInChildren<toolShift>();
    ...
    public void Update()
    {
        if (isShowed)
        {
            ...
            if (Program.InputGetMouseButtonDown_0)
            {
                ...
                {
                    movedFromCollider = false;
                    ES_mouseDownEmpty();
                }
                else
                {
                    movedFromCollider = true;
    ...
    public void RMSshow_yesOrNo(string hashCode, string hint, messageSystemValue yes, messageSystemValue no)
    {
        ...
        if (Program.I().setting.setting.confirmLeft.value)
            ...
            currentMSwindow = create(Program.I().ES_2_right, MSentre(), Vector3.zero, true, Program.I().ui_main_2d);
        UIHelper.InterGameObject(currentMSwindow);
        UIHelper.trySetLableText(currentMSwindow, "hint_", hint);
        UIHelper.registEvent(currentMSwindow, "yes_", ES_RMSpremono, yes);
    ...
    public void RMSshow_yesOrNoForce(string hint, messageSystemValue yes, messageSystemValue no)
    {
        ...
        if (Program.I().setting.setting.confirmLeft.value)
            ...
            yesOrNoForce = create(Program.I().ES_2Force_right, MSentre(), Vector3.zero, true, Program.I().ui_main_2d);
    ...
    public void RMSshow_FS(string hashCode, messageSystemValue first, messageSystemValue second)
    {
        ...
        if (Program.I().setting.setting.confirmLeft.value)
            ...
            currentMSwindow = create(Program.I().ES_FS_right, MSentre(), Vector3.zero, true, Program.I().ui_main_2d);
        UIHelper.InterGameObject(currentMSwindow);
        ...
        messageSystemValue cancle)
    {
        ...
        if (Program.I().setting.setting.confirmLeft.value)
            ...
            currentMSwindow = create(Program.I().ES_3cancle_right, MSentre(), Vector3.zero, true, Program.I().ui_main_2d);
        UIHelper.InterGameObject(currentMSwindow);
        UIHelper.trySetLableText(currentMSwindow, "hint_", hint);
        UIHelper.registEvent(currentMSwindow, "yes_", ES_RMSpremono, yes);
        UIHelper.registEvent(currentMSwindow, "no_", ES_RMSpremono, no);
    ...
    public void RMSshow_singleChoice(string hashCode, List<messageSystemValue> options)
    {
        ...
        for (var i = 0; i < options.Count; i++)
        {
            ...
            UIHelper.trySetLabelText(btn, "[u]" + options[i].hint);
    ...
    public void RMSshow_multipleChoice(string hashCode, int selectCount, List<messageSystemValue> options)
    {
        ...
        var sp = UIHelper.getByName<UI2DSprite>(currentMSwindow, "under");
        sp.height = 50 + UIHelper.get_zonghangshu(options.Count, 5) * 50;
        sp.width = 950;
        for (var i = 0; i < options.Count; i++)
        {
            ...
            (
                ...
                new Vector3(-375 + lie * 150, sp.height / 2 - 50 - 50 * hang, 0),
                Vector3.zero,
                false,
                sp.gameObject,
                false
