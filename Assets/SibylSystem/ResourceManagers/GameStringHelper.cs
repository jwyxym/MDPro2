            ...
            if (data.Id != 0)
            {
                ...
                if ((data.Type & (int)CardType.Monster) > 0 || (origin.Type & (int)CardType.Monster) > 0)
                {
                    if (data.Race != origin.Race)
                        ...
                        re = "【" + race(data.Race) + "族/" + secondType(data.Type) + "】";
                }
                else if ((data.Type & (int)CardType.Spell) > 0)
                    re = "【魔法】";
                else if ((data.Type & (int)CardType.Trap) > 0)
                    re = "【陷阱】";
            }
        }
        catch { }
    ...
    public static int GetSetNameCode(string setName)
    {
        ...
        for (var i = 0; i < GameStringManager.xilies.Count; i++)
        {
            ...
            if (setName == setString)
                returnValue = GameStringManager.xilies[i].hashCode;
        }
        return returnValue;
    }

    public static string GetSpellType(Card data)
    {
        string returnValue = "";
        if (data == null || (data.Type & (uint)CardType.Monster) > 0)
            return returnValue;
        if ((data.Type & (uint)CardType.QuickPlay) > 0)
            ...
            returnValue += "场地";
        else if ((data.Type & (uint)CardType.Counter) > 0)
            returnValue += "反击";
        else if ((data.Type & (uint)CardType.Equip) > 0)
            returnValue += "装备";
