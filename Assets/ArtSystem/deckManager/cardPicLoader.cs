...
public class cardPicLoader : MonoBehaviour
{
    ...
    {
        uiTexture.mainTexture = await GameTextureManager.GetCardPicture(code, player);
    ...
    private void Update()
    {
        if (coli != null && Program.InputGetMouseButtonDown_0 && Program.pointedCollider == coli)
            Program.I().cardDescription.setData(CardsManager.Get(_code), player == 0 ? GameTextureManager.myBack : GameTextureManager.opBack, "", true);
