...
public class toolShift : MonoBehaviour
{
    ...
    public void shift()
    {
        ...
        {
            if (va.y >= 0)
                iTween.MoveToLocal(ObjectMust, new Vector3(va.x, -78, va.z), 0.6f);
            else
                iTween.MoveToLocal(ObjectMust, new Vector3(va.x, 22, va.z), 0.6f);
        }
        else
        {
            ...
            {
                iTween.MoveToLocal(ObjectMust, new Vector3(va.x, -78, va.z), 0.6f);
                iTween.MoveToLocal(ObjectOption, new Vector3(vb.x, 22, vb.z), 0.6f);
            }
            else
            {
                iTween.MoveToLocal(ObjectMust, new Vector3(va.x, 22, va.z), 0.6f);
                iTween.MoveToLocal(ObjectOption, new Vector3(vb.x, -78, vb.z), 0.6f);
            }
        }
    }
}