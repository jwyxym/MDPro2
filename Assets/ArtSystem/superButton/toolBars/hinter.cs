...
public class hinter : MonoBehaviour
{
    ...
    private void in_()
    {
        ...
        if (gameObject.GetComponentInChildren<UITexture>() == null)
            screenPosition = Program.I().camera_main_2d.WorldToScreenPoint(gameObject.transform.position);
        else
            screenPosition = Program.I().camera_main_2d.WorldToScreenPoint(gameObject.GetComponentInChildren<UITexture>().gameObject.transform.position);
        screenPosition.y += 90;
        screenPosition.z = 0;
