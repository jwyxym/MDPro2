...
public class VoiceHandler : MonoBehaviour
{
    ...
    static List<AudioClip> clipList = new List<AudioClip>();
    ...
    private void Update()
    {
        ...
        if (Program.I().ocgcore.gameField != null && Program.I().ocgcore.gameField.gameObject != null)
        {
            ...
            {
                if(Program.I().ocgcore.life_0 / (float)Program.I().ocgcore.lpLimit >= 0.5f)
                    Program.I().ocgcore.gameInfo.SetCharacter0Face(1);
                else
                    Program.I().ocgcore.gameInfo.SetCharacter0Face(7);
                Program.I().DuelCommentMe.Hide();
            }
            //else
            ...
            {
                if (Program.I().ocgcore.life_1 / (float)Program.I().ocgcore.lpLimit >= 0.5f)
                    Program.I().ocgcore.gameInfo.SetCharacter1Face(1);
                else
                    Program.I().ocgcore.gameInfo.SetCharacter1Face(7);
                Program.I().DuelCommentOp.Hide();
            }
            //else
    ...
    public static IEnumerator ABInitAsync()
    {
        ...
        if (ab2 != null)
            ab2.Unload(false);
        AssetBundleCreateRequest _ab1= AssetBundle.LoadFromFileAsync("assetbundle/voice/" + character1.ToLower());
        yield return _ab1;
        ...
        {
            AssetBundleCreateRequest _ab2 = AssetBundle.LoadFromFileAsync("assetbundle/voice/" + character2.ToLower());
            yield return _ab2;
    ...
    public static bool PlayCardLine(uint controller, string type, int card, bool sleep, bool firstOnField = true, uint location = (uint)CardLocation.Onfield, int chain = 1, bool summonline = false)
    {
        if (!Program.I().setting.setting.Voice.value)
        {
            voiceTime = 0;
    ...
    static void PlayGroup(uint controller, AudioClip clip0, bool sleep, bool onlyPlayOne = false, bool hand = false,bool summon = false, int card = 0, bool showText = true)
    {
        ...
        if (sleep)
            Program.I().ocgcore.Sleep((int)(result.length * 60));
        voiceTime = result.length;
        ...
        foreach(AudioClip c in audioClips)
        {
            if(showText)
            {
                string text = VoiceLineHandler.GetVoiceText(controller, c.name);
                if (controller == 0)
                {
                    Program.I().DuelCommentMe.DelaySpeak(waitTime, text);
                    int face = VoiceLineHandler.GetVoiceFace(controller, c.name);
                    if (face == 0) face = 3;
                    Program.I().ocgcore.gameInfo.SetCharacter0Face(face);
                }
                else
                {
                    Program.I().DuelCommentOp.DelaySpeak(waitTime, text);
                    int face = VoiceLineHandler.GetVoiceFace(controller, c.name);
                    if (face == 0) face = 3;
    ...
    public static void PlayFixedLine(uint controller ,string line, bool sleep = true, string cardName = "", bool showText = true)
    {
        ...
        if (clip != null)
            PlayGroup(controller, clip, sleep, false, false, false, 0, false);
        ...
        if(showText)
        {
            ...
            if (clip.name.Substring(6, 2) == "07")
            {
                ...
                query = false;
            if(query)
                text = VoiceLineHandler.GetVoiceText(controller, clip.name);
            ...
            if (controller == 0)
            {
                if (text != "")
                {
                    ...
                    int face = VoiceLineHandler.GetVoiceFace(controller, clip.name);
                    if (face == 0) face = 3;
                    Program.I().ocgcore.gameInfo.SetCharacter0Face(face);
                }
                else
