...
public class UICamera : MonoBehaviour
{
	...
	static public RaycastHit coveredHit;

	static public RaycastHit eventHit;
	...
	{
		coveredHit = new RaycastHit();
		eventHit = new RaycastHit();
        for (int i = 0; i < list.size; ++i)
		{
			UICamera cam = list.buffer[i];
				...
				if (hits.Length > 1)
				{
					...
					for (int b = 0; b < mHits.size; ++b)
					{
...
#endif
						{
                            ...
                            {
                                if (mHits[j].hit.collider.name == lastHit.collider.name && mHits[j].hit.collider != lastHit.collider)
                                    coveredHit = mHits[j].hit;
                            }
                            for (int k = 0; k < mHits.size; k++)
                            {
                                if (mHits[k].hit.collider.name.StartsWith("event_"))
                                    eventHit = mHits[k].hit;
                            }
